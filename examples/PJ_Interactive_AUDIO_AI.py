...
class PJ():
    def __init__(self):
        ...
        # Building text to speech engine
        self.tts_engine = pyttsx3.init()
        """ RATE"""
        rate = self.tts_engine.getProperty('rate')   # getting details of current speaking rate
        print (rate)                        #printing current voice rate
        self.tts_engine.setProperty('rate', 125)     # setting up new voice rate
        voices = self.tts_engine.getProperty('voices')       #getting details of current voice
        #engine.setProperty('voice', voices[0].id)  #changing index, changes voices. o for male
        self.tts_engine.setProperty('voice', voices[1].id)   #changing index, changes voices. 1 for female

        # Building Speech to text engine
    ...
    def listen(self, prompt):
        ...
        while True:
            try:
                print(".",end="")
                audio_data = self.r.listen(self.source,timeout=1)
                text_command = self.r.recognize_google(audio_data, language='en-US')
